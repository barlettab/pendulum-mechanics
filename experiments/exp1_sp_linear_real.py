...
from core.utils import align_series
...
for deg in angulos_graus:
    ...
    # Simulações
    theta_real = simulate_simple_pendulum(rad, 0, dt=dt)
    theta_pred = simulate_linear_pendulum(rad, 0, dt=dt, steps=len(theta_real))
    
    # Métrica de erro (Theil U2) com alinhamento explícito de séries
    theta_real_aligned, theta_pred_aligned = align_series(theta_real, theta_pred)
    U = theils_u(theta_real_aligned, theta_pred_aligned)
    ...
    np.save(f"data/exp1/theta_real_{deg}.npy", theta_real)
    np.save(f"data/exp1/theta_pred_{deg}.npy", theta_pred)
    
    # Geração do Gráfico com vetor de tempo no eixo X
    vetor_tempo = np.arange(len(theta_real)) * dt
    
    plt.figure(figsize=(10, 4))
    plt.plot(vetor_tempo, theta_real, label=r"Pêndulo Real (Não Linear - $\sin(\theta)$)", color="#2ca02c", linewidth=2)
